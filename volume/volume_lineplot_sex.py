...
for sex_val in [0, 1]:
    ...
    # Linear regression
    model = smf.ols("volume ~ diab + age + C(college)", data=sdf)
    results = model.fit()
...
# Linear regression
model = smf.ols("volume ~ diab + sex + diab*sex + age + C(college)", data=sdf)
results = model.fit()
# print(results.summary())
...
# Title
plt.title("Gray Matter Volume across Age, T2DM Status and Sex\n" \
          f"N={ss} (Exact Matched)", x=0.42)
